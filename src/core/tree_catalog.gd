...
extends RefCounted
## Single source of truth for the tree sprite sheet layout (#234 art pass).
## Mirrors tools/gen_trees.py: 4 variants in one row of CELL_W x CELL_H cells,
## each with the trunk base at ANCHOR and a long SE cast shadow baked into the
## art (sun from NW). Trees follow the same placement contract as every entity:
## node origin at the footprint (Y-sort key), art offset lifts it onto the tier.
