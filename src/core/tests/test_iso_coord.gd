...
func _test_round_trip() -> void:
	...
	for c in cells:
		...
		_ok(back == c, "round-trip iso_to_cart(cart_to_iso(%s)) == %s got %s" % [c, c, back])
		var picked := IsoCoord.pick_cell(IsoCoord.cart_to_iso(c))
		_ok(picked == c, "pick_cell(cart_to_iso(%s)) == %s got %s" % [c, c, picked])

## The four canonical cart_to_iso values (W=64, H=32).
func _test_exact_values() -> void:
	_ok(_v_approx(IsoCoord.cart_to_iso(Vector2i(0, 0)), Vector2(64, 32)), "cart_to_iso(0,0)=(64,32)")
	_ok(_v_approx(IsoCoord.cart_to_iso(Vector2i(1, 0)), Vector2(128, 64)), "cart_to_iso(1,0)=(128,64)")
	_ok(_v_approx(IsoCoord.cart_to_iso(Vector2i(0, 1)), Vector2(0, 64)), "cart_to_iso(0,1)=(0,64)")
	_ok(_v_approx(IsoCoord.cart_to_iso(Vector2i(1, 1)), Vector2(64, 96)), "cart_to_iso(1,1)=(64,96)")
...
func _test_point_in_diamond() -> void:
	...
	_ok(IsoCoord.is_point_in_diamond(center, cell), "center inside diamond of %s" % cell)

	# Just past the bottom vertex (H + 2 px below center): outside.
	var outside := center + Vector2(0, h + 2.0)
	_ok(not IsoCoord.is_point_in_diamond(outside, cell), "point past vertex is outside %s" % cell)
	...
	for v in verts:
		_ok(IsoCoord.is_point_in_diamond(v, cell), "vertex %s on boundary of %s" % [v, cell])
...
func _test_pick_cell_edges() -> void:
	...
	_ok(IsoCoord.pick_cell(toward_10) == Vector2i(1, 0), "edge point %s -> (1,0) got %s" % [toward_10, IsoCoord.pick_cell(toward_10)])
	_ok(IsoCoord.pick_cell(toward_00) == Vector2i(0, 0), "edge point %s -> (0,0) got %s" % [toward_00, IsoCoord.pick_cell(toward_00)])
	...
	_ok(IsoCoord.pick_cell(near_vertex) == Vector2i(2, 2), "near-vertex %s -> (2,2) got %s" % [near_vertex, IsoCoord.pick_cell(near_vertex)])
...
func _test_elevation_single_source() -> void:
	for level in range(0, 4):
		...
		_ok(off == want, "elevation_offset(%d) == %s got %s" % [level, want, off])
