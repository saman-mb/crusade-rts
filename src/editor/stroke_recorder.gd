...
## behind a set_cell(cell, source_id, atlas_coords)-shaped `sink` Callable. Follows the
## "empty is -1, never 0" rule (BrushCore.EMPTY_SOURCE_ID); source_id -1 means clear.

## Backing store, keyed by cell. Each value is
## { "before_src": int, "before_atlas": Vector2i, "after_src": int, "after_atlas": Vector2i }.
## The before-state is first-seen-wins; the after-state is last-write-wins.
...
func record(cell: Vector2i, before_src: int, before_atlas: Vector2i, after_src: int, after_atlas: Vector2i) -> void:
	if _cells.has(cell):
...
## { "cell", "before_src", "before_atlas", "after_src", "after_atlas" }. Drops no-op
## records AND paint-then-reverted-within-stroke cells (net zero). Order is not
## guaranteed but is deterministic per run (iterates the backing dict's insertion order).
func changes() -> Array:
	var out: Array = []
	for cell in _cells:
		var e: Dictionary = _cells[cell]
		if e["before_src"] == e["after_src"] and e["before_atlas"] == e["after_atlas"]:
			continue
		out.append({
			"cell": cell,
...
func is_empty() -> bool:
	for cell in _cells:
		var e: Dictionary = _cells[cell]
		if e["before_src"] != e["after_src"] or e["before_atlas"] != e["after_atlas"]:
			return false
	return true
...
## do-method binding the AFTER state and an undo-method binding the BEFORE state, then
## commit_action(false) ONCE. Returns early (no action created) on an empty change set.
## execute=false because the caller applied the stroke live -- see commit() above.
static func apply_to_undo(ur: UndoRedo, action_name: String, changes: Array, sink: Callable) -> void:
	...
	for c in changes:
		...
		ur.add_do_method(sink.bind(cell, c["after_src"], c["after_atlas"]))
		ur.add_undo_method(sink.bind(cell, c["before_src"], c["before_atlas"]))
