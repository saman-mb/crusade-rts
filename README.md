...
**Scattered doodads** (`assets/doodads/doodads.png`, drawn by `tools/gen_doodads.py`)
pepper the ground with rocks, bushes, grass tufts, flowers, and pebbles so it reads
as *designed* terrain rather than a bare field. Placement is deterministic
(`DoodadScatter` → a hash of each cell, re-derived on load, never persisted) and the
sprites Y-sort like any unit — origin at the footprint, art lifted onto the tier via
`EntityPlacement`. The doodad art is procedurally generated and project-owned (Story
#234; see [CREDITS.md](CREDITS.md)).
